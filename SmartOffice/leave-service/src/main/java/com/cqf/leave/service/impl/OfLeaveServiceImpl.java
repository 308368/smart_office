package com.cqf.leave.service.impl;
...
import com.cqf.leave.enums.LeaveDays;
import com.cqf.leave.model.dto.AddLeaveDTO;
import com.cqf.leave.model.po.OfLeave;
import com.cqf.leave.mapper.OfLeaveMapper;
import com.cqf.leave.model.vo.LeaveVo;
import com.cqf.leave.service.IOfLeaveService;
...
public class OfLeaveServiceImpl extends ServiceImpl<OfLeaveMapper, OfLeave> implements IOfLeaveService {
    ...
    private static void calculateDays(Map<String, Integer> result) {
        ...
        if (yearLeave != null) {
            result.put("年假", LeaveDays.YEARLY_LEAVE.getDays() - yearLeave);
        }else {
            result.put("年假", LeaveDays.YEARLY_LEAVE.getDays());
        }
        Integer sickLeave = result.get("病假");
        if (sickLeave != null) {
            result.put("病假", LeaveDays.SICK_LEAVE.getDays() - sickLeave);
        }else{
            result.put("病假", LeaveDays.SICK_LEAVE.getDays());
        }
        Integer maternityLeave = result.get("产假");
        if (maternityLeave != null) {
            result.put("产假", LeaveDays.MATERNITY_LEAVE.getDays() - maternityLeave);
        }else {
            result.put("产假", LeaveDays.MATERNITY_LEAVE.getDays());
        }
        Integer marriageLeave = result.get("婚假");
        if (marriageLeave != null) {
            result.put("婚假", LeaveDays.MARRIAGE_LEAVE.getDays() - marriageLeave);
        }else {
            result.put("婚假", LeaveDays.MARRIAGE_LEAVE.getDays());
        }
    }
}
